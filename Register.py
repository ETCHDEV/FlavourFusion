...
import subprocess
import re  # <-- Ensure the 're' module is imported for regex validation
...
def register_user():
    username = entry_1.get().strip()  # .get() for Entry widget (Username)
    email = entry_2.get().strip()     # .get() for Entry widget (Email)
    password_ = entry_3.get().strip()  # .get() for Entry widget (Password)
    phone = entry_4.get().strip()     # .get() for Entry widget (Phone No)
    ...
    try:
        # Insert data including phone number
        cursor.execute("INSERT INTO users (username, email, password_hash, phone) VALUES (%s, %s, %s, %s)", (username, email, password_, phone))
        db.commit()
        open_file()
...
# Setup tkinter window for registration
window = tk.Tk()
window.geometry("578x658")
window.configure(bg="#FFFFFF")

# Function to handle relative paths for assets
...
# Background image and input fields
...
# Add the "Register" text above the email input field (Centered)
canvas.create_text(289, 240, anchor="center", text="Register", fill="#000000", font=("Arial", 20, "bold"))

# Username input field (Changed to Entry)
...
# Email input field (Changed to Entry)
...
entry_3 = tk.Entry(window, bd=0, bg="#CFB9B9", fg="#000716", highlightthickness=0)
...
# Phone input field (Changed to Entry)
...
# Close Button
close_button = tk.Button(window, text="X", font=("Arial", 16), bg="red", fg="white", command= lambda: open_file(), relief="flat")
close_button.place(x=550, y=10)
